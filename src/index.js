...
* Ввести необходимые данные в форму.
  ...
  - Надо исправить:
    ...
    - "После успешной регистрации должен происходить редирект на страницу входа"
      ...
      - В коде в onRegister стоит переход history.push("/projects");
        Косвенно за счет других настроек будет переход на страницу '\login' но это затрудняет понимание результатов работы кода.
        Рекомендую делать явные переходы.
...
*** Общие замечания ***
  ...
  - Что было сделано отлично
    - ProtectedRoute - хорошее решение через новый отдельный компонент, которое можно применять для новых страниц сайта
  ...
  - Можно лучше:
    ...
    - onLogin/onRegister/onSignout/UserContext выглядят как связанные сущности.
      Их можно вынести в новый компонент, использовать его вместо UserContext.Provider 
      и получать все эти значения через useContext(UserContext)
